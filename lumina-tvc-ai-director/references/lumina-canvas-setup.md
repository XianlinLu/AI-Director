...
为了显示原生单选 UI，Lumina 运行时需要向 Agent 暴露“用户选择 / 交互式提问 / 单选表单”能力。不同环境的工具名称可能不同，Skill 按能力描述选择，不依赖固定名称。
...
2. 检查用户是否已经明确给出唯一选题。
3. 未给出时，调用交互式单选能力。
4. 传入一个本地化问题、四个动态建议选项，并启用系统 Other 自填项。
5. 用户提交后锁定 `TOPIC_DIRECTION` 并继续创意开发。

四个选项必须根据当前用户的产品、目标、受众、情绪和限制实时生成，不使用固定选题库。每项包含短标签和一句简短说明；四项必须属于同一决策层级且真正不同。

如果运行时会自动附加 Other，只传四个建议项；如果必须显式开启 Other，则启用自由文本字段。不得生成两个 Other。若没有交互式选择能力，Skill 才会回退为文本形式的四个选项加一个自填入口。
...
```text
我要为一款面向刚入职年轻人的便携咖啡机做一条 20 秒竖屏短片，核心想法是“在工位上迅速找回自己的节奏”，不要家庭场景。请先让我选择选题方向。
```

预期：Agent 不复用任何固定故事题材；弹窗问题复述用户的实际目标，给出四个都与“职场新人、工位、便携咖啡机、找回节奏、竖屏”相关且互相区别的选题，并显示一个可自由填写的 Other。选择前不进入完整提案或生成。
